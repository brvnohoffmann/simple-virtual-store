    ...
    <div class="items">
      ...
      <template v-if="productsInBag.length">
        ...
        <div v-for="(product, index) in productsInBag" v-bind:key="index" class="item">
          <div class="remove" v-on:click="this.$store.dispatch('removeFromBag', product.id)">Remover Produto</div>
          <div class="photo"><img v-bind:src="product.image" alt=""></div>
          <div class="description">{{ product.title }}</div>
          <div class="price">
            <span class="quantity-area">
              <button v-bind:disabled="product.quantity <= 1" v-on:click="product.quantity--">-</button>
